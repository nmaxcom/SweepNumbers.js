...
let Sweep = (opts = {}) => {
    ...
    let validCursors = ['auto', 'default', 'context-menu', 'help', 'pointer', 'progress', 'wait', 'cell', 'crosshair', 'text', 'vertical-text', 'alias', 'copy', 'move', 'no-drop', 'not-allowed', 'all-scroll', 'col-resize', 'row-resize', 'n-resize', 'e-resize', 's-resize', 'w-resize', 'ns-resize', 'ew-resize', 'ne-resize', 'nw-resize', 'se-resize', 'sw-resize', 'nesw-resize', 'nwse-resize'];
    ...
    if ($sweeps.length === 0) {
        console.log(`%cSweepNumbers: We didn't find any elements with the selector '${options.selector}'`, 'color:white;background-color:' + options.color + ';padding:3px');
        return;
    }
    let startingX, // x coordinate of the mouse when started dragging
        ...
        currentValue, // let's not lose track of the real int number without decorations and splits
    ...
    for (let i = 0, len = $sweeps.length; i < len; i++) {
        ...
        if (options.clickEditable) $sweeps[i].addEventListener('click', clickF);
        $sweeps[i].addEventListener('keypress', keypressF);
        $sweeps[i].addEventListener('blur', blurF);
        $sweeps[i].value = markupValue($sweeps[i].value);
     ...
     */
    function mouseoverF() {
        if (options.cursor && validCursors.includes(options.cursor)) {
            document.body.style.cursor = options.cursor;
            event.target.style.cursor = options.cursor;
        } else
            document.body.style.cursor = options.cursor;
    }
    ...
    function mousemoveF(event) {
        if (dragging) {
            ...
            currentValue = parseInt(originalNumber) + moved;
            target.value = (options.onlyPositive && currentValue < 0) ? 0 : markupValue(currentValue);
            updateWidth(target);
        }
    }
    /**
    ...
    function markupValue(value) {
        if (options.splitThousands) value = value.toLocaleString();
